    ...
    function setTitle() {
      if (window.matchMedia('( min-width: 768px )').matches) {
        ...
        $('.b-catalog-element').each(function () {
          ...
          if (String($title.text()).length > 60) {
            $title.text($.trim(String($title.text()).substring(0, 59)) + '...');
          }
        });
      } else {
        //mobile
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data && data.STATUS === 'Y') {
            ...
            if (left > 0) {
              $('#buyCatalogElementPopupWindow').removeClass('i-inverse');
              $('#buyCatalogElementPopupMin').text(min.toLocaleString());
              $('#buyCatalogElementPopupLeft').text(left.toLocaleString());
            } else {
              $('#buyCatalogElementPopupWindow').addClass('i-inverse');
              $('#buyCatalogElementPopupOrder').text(order.toLocaleString());
            }
