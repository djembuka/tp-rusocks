(function ($) {
  ...
  $(function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data && data.STATUS === 'Y') {
            ...
            //show price and buttons
            var min = data.MIN;
            var order = data.ORDER;
            var left = min - order;

            if (left > 0) {
              $('#buyDetailPopupWindow').removeClass('i-inverse');
              $('#buyDetailPopupMin').text(min.toLocaleString());
              $('#buyDetailPopupLeft').text(left.toLocaleString());
            } else {
              $('#buyDetailPopupWindow').addClass('i-inverse');
              $('#buyDetailPopupOrder').text(order.toLocaleString());
            }
    ...
    if (document.querySelector('.b-catalog-detail__gallery-simple')) {
      document
        .querySelector('.b-catalog-detail__gallery-simple')
        .addEventListener('click', function (e) {
          var item = e.target.classList.contains(
            'b-catalog-detail__gallery-item'
          )
            ? e.target
            : e.target.closest('.b-catalog-detail__gallery-item');
          if (item) {
            e.preventDefault();
            var itemsArray = Array.from(
              document.querySelectorAll('.b-catalog-detail__gallery-item')
            );
            var imgIndex = itemsArray.findIndex(
              (itemElement) => itemElement === item
            );
            createBigGallery(imgIndex);
          }
        });
    }

    if (document.querySelector('.b-catalog-detail__gallery-big__close')) {
      document
        .querySelector('.b-catalog-detail__gallery-big__close')
        .addEventListener('click', function (e) {
          e.preventDefault();
          //clear the gallery
          document.querySelector(
            '.b-catalog-detail__gallery-big.swiper .swiper-wrapper'
          ).innerHTML = '';
          //remove fullscreen
          document
            .querySelector('body')
            .classList.remove('b-catalog-detail__gallery-big--fullscreen');
        });
    }

    function createBigGallery(imgIndex) {
      var images = '',
        srcBigArray,
        body = document.querySelector('body'),
        galleryBigElement = document.querySelector(
          '.b-catalog-detail__gallery-big.swiper'
        ),
        swiper;
      ...
      //images html
      srcBigArray.forEach(function (cur) {
        images += `<div class="swiper-slide"><img src="${cur}" alt="" ></div>`;
      });

      if (galleryBigElement) {
        //images
        galleryBigElement.querySelector('.swiper-wrapper').innerHTML = images;

        //video
        if (document.getElementById('catalogDetailVideo')) {
          $('.b-catalog-detail__gallery-big.swiper .swiper-wrapper').append(
            $('#catalogDetailVideo').html()
          );
        }

        //update
        swiper = galleryBigElement.swiper;

        if (swiper) {
          swiper.update();
        } else {
          body.append(galleryBigElement);
          swiper = new Swiper(galleryBigElement, {
            slidesPerView: 1,
            navigation: {
        ...
        //fullscreen
        body.classList.add('b-catalog-detail__gallery-big--fullscreen');
      }
    }

    function destroyBigGallery() {
      if ($('.b-catalog-detail__gallery .fotorama').length) {
        var $fotorama = $('.b-catalog-detail__gallery .fotorama');
    ...
    function showOffer($div) {
      ...
      //gallery
      var gallerySimpleElement = document.querySelector(
          '.b-catalog-detail__gallery-simple'
        ),
        gallerySimpleWrapper = gallerySimpleElement.querySelector(
          '.b-catalog-detail__gallery-simple-wrapper'
        ),
        gallerySimplePagination = gallerySimpleElement.querySelector(
          '.b-catalog-detail__gallery-simple-pagination'
        ),
        images = '',
        srcArray = [],
      ...
      if (isMobile) {
        gallerySimpleElement.classList.add('swiper');
        gallerySimpleWrapper.classList.add('swiper-wrapper');
        gallerySimplePagination.classList.add('swiper-pagination');
      } else {
        gallerySimpleElement.classList.remove('swiper');
        gallerySimpleWrapper.classList.remove('swiper-wrapper');
        gallerySimplePagination.classList.remove('swiper-pagination');
      }
      ...
      srcArray.forEach(function (cur, i, arr) {
        images += `<div class="b-catalog-detail__gallery-item ${
          isMobile ? 'swiper-slide' : ''
        }"><img class="b-catalog-detail__gallery-img" src="${cur}" alt=""><img class="b-catalog-detail__gallery-zoom" src="/components/catalog.detail.simple-gallery/img/search-zoom-in.svg"></div>`;
      });

      if (gallerySimpleElement) {
        gallerySimpleWrapper.innerHTML = images;
        //video
        if (document.getElementById('catalogDetailVideo')) {
          $('.b-catalog-detail__gallery-simple-wrapper').append(
            `<div class="b-catalog-detail__gallery-video ${
              isMobile ? 'swiper-slide' : ''
            }">${$('#catalogDetailVideo').html()}</div>`
          );
        }

        if (isMobile) {
          var swiper = new Swiper(gallerySimpleElement, {
            slidesPerView: 1.2,
            spaceBetween: 1,
            pagination: {
      ...
      if ($('.b-catalog-detail__old-price').length) {
        if ($div.data('old-price')) {
          $('.b-catalog-detail__old-price')
            .show()
            .find('s')
            .html($div.data('old-price'));
          $('.b-catalog-detail__price').addClass('i-sale');
        } else {
          $('.b-catalog-detail__old-price').hide();
          $('.b-catalog-detail__price').removeClass('i-sale');
        }
      }
      //discount
      var discountText = $('.b-catalog-detail__benefit span').html();
      if ($div.data('discount-diff')) {
        $('.b-catalog-detail__benefit')
          .show()
          .html(
            '<span>' + discountText + '</span>' + $div.data('discount-diff')
          );
      } else {
